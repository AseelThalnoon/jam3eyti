/* جمعيتي v1.2 — Hide edit/add/schedule when no jamiyah open
   - Sections (editBlock, addMemberBlock, membersBlock, scheduleBlock) are hidden unless a jamiyah is open
   - Keeps: collapsible sections, vivid buttons, month input, PDF export, scrollIntoView on open
*/
...
/* Gate all details sub-sections by whether a jamiyah is open */
function setDetailsSectionsVisible(hasOpen){
  // المطلوب: إخفاء هذه الأقسام إذا لا يوجد جمعية مفتوحة
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* أول تحميل: لا جمعية مفتوحة → اخفِ الأقسام */
...
/* ---------- Open details (scroll into view) ---------- */
function openDetails(id){
  state.currentId = id;
  const j = currentJamiyah();

  if (!j) {
    // لا جمعية: اخفِ الأقسام
    $('#details').classList.add('hidden');
    setDetailsSectionsVisible(false);
    return;
  }
  ...
  // أظهر الأقسام لأن فيه جمعية مفتوحة الآن
  setDetailsSectionsVisible(true);

  // Responsive default for collapsibles
...
function onSaveEdit(e){
  ...
  const startMonth = $('#e-start').value;   // YYYY-MM
  ...
  const scheduleRows = Array.from({length: j.duration}, (_,k)=>{
    ...
    return `
      <tr>
        ...
        <td>${receiversText}</td>
        <td>Spent: ${fmtMoney(totalAssigned)} · Left: ${fmtMoney(remaining)}</td>
      </tr>
    `;
  }).join('');
...
/* ---------- Back ---------- */
function showList(){
  // إخفِ كامل تفاصيل الجمعية
  $('#details').classList.add('hidden');
  // ولا تُظهر الأقسام التابعة لأن لا توجد جمعية مفتوحة الآن
  setDetailsSectionsVisible(false);
}